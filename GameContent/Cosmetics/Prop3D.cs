...
public class Prop3D(string name, Resource<Model> model, Texture2D texture, Vector3 position, PropLockOptions lockOptions) : IProp {
    ...
    public Resource<Model> PropModel { get; set; } = model;
    ...
    public object Clone() {
        return new Prop3D(Name, PropModel, ModelTexture, RelativePosition, LockOptions) {
            IgnoreMeshesByName = this.IgnoreMeshesByName,
            Scale = this.Scale,
