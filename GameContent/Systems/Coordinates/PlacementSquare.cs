                    ...
                    if (BlockId > -1) {
                        if (displayHeights && Block.AllBlocks[BlockId] is not null) {
                            if (Block.AllBlocks[BlockId].Properties.CanStack) {
                                ...
                                DrawUtils.DrawBorderedText(TankGame.SpriteRenderer, TankGame.TextFont, $"{Block.AllBlocks[BlockId].Stack}", pos, Color.White, Color.Black, new Vector2(TankGame.AddativeZoom * 1.5f).ToResolution(), 0f, Anchor.Center);
                            }
                            if (Block.AllBlocks[BlockId].Type == BlockID.Teleporter) {
                                var pos = MatrixUtils.ConvertWorldToScreen(Vector3.Zero, effect.World, effect.View, effect.Projection);

                                DrawUtils.DrawBorderedText(TankGame.SpriteRenderer, TankGame.TextFont, $"TP:{Block.AllBlocks[BlockId].TpLink}", pos, Color.White, Color.Black, new Vector2(TankGame.AddativeZoom * 1.5f).ToResolution(), 0f, Anchor.Center);
                            }
                        }
                    }
                    else if (TankId > -1) {
                        ...
                        if (GameHandler.AllTanks[TankId] is PlayerTank player)
                            DrawUtils.DrawBorderedText(TankGame.SpriteRenderer, TankGame.TextFont, $"ID: {player.PlayerId}", pos + new Vector2(0, 8), TeamID.TeamColors[GameHandler.AllTanks[TankId].Team], Color.Black, new Vector2(0.8f).ToResolution() * TankGame.AddativeZoom, 0f, Anchor.Center);
                    }
                }
