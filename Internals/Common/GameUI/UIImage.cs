...
public class UIImage(Texture2D texture, Vector2 scale, Action<UIImage, SpriteBatch>? uniqueDraw = null) : UIElement {
    public Texture2D Texture { get; set; } = texture;
    public Vector2 Scale { get; set; } = scale;
    public Color Color { get; set; } = Color.White;

    public Action<UIImage, SpriteBatch>? UniqueDraw = uniqueDraw;

    public override void DrawSelf(SpriteBatch spriteBatch) {
        base.DrawSelf(spriteBatch);

        if (UniqueDraw is null && Texture is not null) {
    ...
    public void OverrideHitbox(Rectangle rectangle) {
